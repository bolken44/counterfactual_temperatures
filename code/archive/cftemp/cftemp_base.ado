...
program define cftemp_base
    ...
    * Catch the syntax.
    syntax varlist(min=4 max=4) [, binsize(real 5) lb(real -10) ub(real 35) time(varlist) keep(string) trend(string) bayes(string) splines aggregate(real 5) realonly]
	...
	/* quietly { */
		...
		* Empirical Bayes
		...
		that these parameters be shrinked toward the average of that parameter for `geo' units 
		inside the specified [geographic variable], presumed to be a larger geographic unit than `geo'.
		In particular, bayes(mean, all) specifies that the parameters be shrinked toward the
		average of all geographic units.
		The shrinkage is by inverse-variance weighting.

		bayes(zero) specifies that the parameters be shrinked toward zero.
		*/
		...
		* Linear trends
		...
		trend(avg_temp, [time variable]) specifies that the average temperature of each time1-geo is linearly regressed against the average temperature of all days and geographic units in each [time variable]. Note that only specifying [month] which takes the values 1-12 is likely insufficient as what you mean
		is probably [year, month].
		...
		* Linear trend in year (should be specified trend(year))
		if "`trend'" == "`time2'" {
			local trendvar = "event_`time2'"
			local mintrendvar = "mintime2"
		}

		* Linear trend in national/global avg temperature of the year/month (specified in trend_param)
		if "`trend_ff'" == "avg_temp" {
			bysort `trend_param': egen meantemp = mean(`temp')
			egen min_meantemp = min(meantemp)
			replace meantemp = meantemp - min_meantemp
			local trendvar = "meantemp"
			local mintrendvar = "min_meantemp"
		}
		...
		** Make the necessary time variables
		...
		If 'month' is specified in time(), this codes a unique signifier
		for each year-month. This will simply be year if time(year). */
		
		*******************************************************************************/
		** Realized Temperature Bins
		/* This should be at the level of `geo' and the level of the return dataset, so
		compute everything by `geo' uni_time. 
		...
		for the counterfactual bins below, simply switch "real_" to "exp_".
		...
		** Counterfactual Temperature Bins
		if "`realonly'" == "" {
			...
			****************************************
			* Linear trends
			if strpos("`trend_ff'", "cheb") <= 0 & "`splines'" == "" {
				...
				* If there are less than 2 observation per geo-time1 (e.g., only 1 year of data for county-month), cannot regress. Mark those
				/* bysort `geo' `time1' `time2': gen `time2'_tag = _n == 1
				bysort `geo' `time1': egen `time2'_count = total(`time2'_tag) */
				...
				if `aggregate' != . {
					...
					forval y = `r(min)'/`r(max)' {
						...
						* store number of times slope needs to be added (removed) from daily temp
						gen slopeTimes`y' = .
						forval i = -`aggregate'/`aggregate' {
							replace slopeTimes`y' = `i' if event_`time2' == `y' + `i' // missing for edge years
						}

						* only use temperatures from the specified period
						local lowerBound = `y' - `aggregate'
						local upperBound = `y' + `aggregate'
						gen `temp'_`y' = `temp' if (inrange(event_`time2',`lowerBound',`upperBound') & event_`time2' != `y')	
						
						* move daily temp according to slope
						replace `temp'_`y' = `temp'_`y' - slopeTimes`y' * slope // missing for edge years
					}
				}
			}
			...
			* Splines
			if "`splines'" != "" {
				...
				* Test for structural breaks
				foreach g in `geocode' {
					...
					preserve
						...
						xtset `geo' event_`time2'

						* Find optimal number of kinks
						xtbreak avg_temp_year event_`time2' if uniqueByYear == 1 & `geo' == `g'
						local num_breaks = e(num_breaks)
						
						* Find location of those breaks and store them
						xtbreak estimate avg_temp_year event_`time2' if uniqueByYear == 1 & `geo' == `g', breaks(`num_breaks')
					...
					local knotlist = ""
					forval k = 1/`num_breaks' {
						local knot`k' = kink[2,`k']
						cap gen knot`k' = .
						replace knot`k' = `knot`k'' if `geo' == `g'
						local knotlist = "`knotlist' `knot`k''"
					}
					dis "`knotlist'"
					...
					if `num_breaks' > 0 {
						* Make the splines
						makespline linear event_`time2' if `geo' == `g', knotslist(`knotlist') replace

						forval k = 1/`num_breaks' {
							cap gen spline`k' = 0
							replace spline`k' = _sp_1_`k' if `geo' == `g'
							sum spline`k'
							local splinelist = "`splinelist' _sp_1_`k'"
						}
					}

					* estimate regressions and slopes for splines
					forvalues t = `mintime1'/`maxtime1' {
						regress avg_temp event_`time2' `splinelist' if `time1' == `t' & `geo' == `g' & unique == 1
						cap gen slope0 = 0
						replace slope0 = _b[event_`time2'] if `time1' == `t' & `geo' == `g'

						* skipped if num_breaks = 0
						forval k = 1/`num_breaks' {
							cap gen slope`k' = 0
							replace slope`k' = _b[_sp_1_`k'] if `time1' == `t' & `geo' == `g'
						}
					}	
				}
				...
				* Determine the max number of breaks
				local trend_sum = ""
				sum num_breaks
				forval k = 1/`r(max)' {
					local trend_sum = "`trend_sum' - slope`k' * spline`k'"
				}
				dis "`trend_sum'"

				gen detrendedTemp = `temp' - slope0 * event_`time2' `trend_sum'
			...
			* Loop through the trend variable  and move the aggregate distribution one slope at a time
			...
			foreach y of local event_`time2'values {
				...
				* Add one slope back to the aggregate distribution
				if strpos("`trend_ff'", "cheb") <= 0 & "`splines'" == "" & `aggregate' == . {
					gen retrendvar = .
					...
					local retrend_sum = "+ slope0 * `y'"
					sum num_breaks
					forval k = 1/`r(max)' {
						local retrend_sum = "`retrend_sum' + slope`k' * (`y' > knot`k') * spline`k'"
					}
					gen retrendedTemp = detrendedTemp `retrend_sum' 
				}
				else if `aggregate' != . {
					gen retrendedTemp = `temp'_`y'
				}
				dis "`retrend_sum'"
				...
				* Plot the cftemps
				/* foreach fip in 25025 4013 {
					...
					twoway ///
						(line retrendedTemp doy, lcolor(blue)), ///
						title("Counterfactual Tempearture Distribution") ///
						ytitle("Predicted Temperature") ///
						xtitle("Day of Year")
					graph export "/proj/pbolken/climate/Haru/output/checks/year/fit_`fip'_`y'_era5_F.pdf", replace
					restore
				} */
		...
		** Organize and return
		...
		geographic unit - return time level are redundant. Drop those dupicates. */
