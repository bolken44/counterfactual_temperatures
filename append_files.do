...
foreach source in schlenker_F { //era5_F schlenker_F ghcn_ext prism_1970

      // Define output file name
      local output_tex "${output}bindev/cftemp_comp_`source'.tex"
      local output_csv "${output}bindev/cftemp_comp_`source'.csv"
      ...
      forvalues i = 1/12 {
            ...
            *** TEXs
            foreach bin in under_10 over_90 {
                  file open in using "${output}bindev/cftemp_comp_`source'_`j'_`bin'.tex", read text
                  file read in line
            ...
            *** CSVs
            ...
            /* shell rm -f "${output}/bindev/cftemp_`source'_linear_`j'.csv" */
      }
...
*******************************************
      ...
      * Plot distribution of `coef`
      ...
      foreach bin in under_10 over_90 {
            ...
            graph export "${output}bindev/coef_plot_naive_`bin'_`source'.pdf", replace
            ...
            , ///
                  ...
                  xtitle("Coefficient", size(small)) ///
                  ytitle("Density", size(small))
            graph export "${output}bindev/coef_plot_cftemp_`bin'_`source'.pdf", replace
            ...
            * Plot distribution of `tstat`
            ...
            , ///
                  ...
                  ytitle("Density", size(small))
            graph export "${output}bindev/tstat_plot_naive_`bin'_`source'.pdf", replace
            ...
            , ///
                  ...
                  xtitle("t-Stats", size(small)) ///
                  ytitle("Density", size(small))
            graph export "${output}bindev/tstat_plot_cftemp_`bin'_`source'.pdf", replace
