...
{phang}
The `cftemp` program takes a temperature dataset at the (say) place-day level and transforms it to an aggregated dataset (say, place-year level) with variables holding the {it:realized} and {it:expected} number of days that fall in each of a user-specified set of temperature bins.
...
{p 8 17 2}
{cmdab:cftemp} {it:tempvar} {it:geovar} {it:timevar1} {it:timevar2}, [{opt binsize(#)} {opt lb(#)} {opt ub(#)} {opt time(var)} {opt parallel(#)}]

{synoptset 15 tabbed}
{synopt:{opt tempvar}} The variable holding temperature data. {p_end}
{synopt:{opt geovar}} The variable uniquely indexing the geographic unit. {p_end}
{synopt:{opt timevar1}} The time variable for which temperature distributions are calculated separately (e.g., month) {p_end}
{synopt:{opt timevar2}} The time variable across which temperature is assumed to follow a linear trend (e.g., year) {p_end}
...
{synopt:{opt time(var)}} The variable used for aggregation. Default is `year`. {p_end}
...
{synopt:{space 4}{opt bayes(mean, geovar)}} Shrinks parameters toward the average of that parameter within the specified geographic variable (presumed to be a larger geographic unit, e.g., states if {it:geovar} is county). {p_end}
...
{title:Example:}
{p 8 17 2}
{cmdab:cftemp} avg_temp_day fips month year, binsize(5) lb(-10) ub(35) agg(year)
...
{phang}
Specifying {bf:agg(month)} will instead return a fips-year-month level dataset with the number of days in each month for each bin. {p_end}
