...
program define cftemp_plot
	...
	local above_str = cond(`ub' < 0, "n`=abs(`ub')'", "`ub'") //to name the bin real_over_`ub'
      ...
      *** Plot results with and without correction together
      ...
      if `version_num' == 2 {
            graph tw (scatter coef1 variable1, color("31 88 137")) (rcap lb1 ub1 variable1, color("31 88 137")) (scatter coef2 variable2, color("155 52 58")) (rcap lb2 ub2 variable2, color("155 52 58")), xlabel(`bin_labels', labsize(small)) xtitle("") yline(0, lpattern(dash) lcolor(red)) ylabel(`ylabels', angle(h)) legend(order(1 "`title0'" 3 "`title1'") position(6) rows(1) `legendhet') `graph'
      }
      if `version_num' == 1 {
            graph tw (scatter coef1 variable1, color("31 88 137")) (rcap lb1 ub1 variable1, color("31 88 137")), xlabel(`bin_labels', labsize(small)) xtitle("") yline(0, lpattern(dash) lcolor(red)) ylabel(`ylabels', angle(h)) legend(order(1 "`title0'") position(6) rows(1) `legendhet') `graph'
      }
